...
Page({
  ...
  showIco: function (e) {
    ...
    console.log(this.data.chooseCompany)
  },
  //确认申报
  confirmDeclareC: function () {
    var that = this
    if (that.data.chooseCompany.length == 0) {
      ...
      for (var i = 0; i < that.data.chooseCompany.length; i++) {
        ...
        openapi.dorequest(parmaers, 'sjt.tax.ybnsr0sb', (res) => {
          /**************************************************************** */
          /**************************************************************** */
          //成功之后执行
          openapi.dorequest(parmaer, 'applet.declare.add.record', (res) => {
            ...
            wx.redirectTo({
              url: 'sucfail/sucfail?' + "&arrAdd=" + companyL + "&arrAddy=" + yuebao + "&arrAddj=" + jibao
            })
          })
        })
      }
